"""
Multi-line draft-PO (delivered, inventory) flow: sync PO to DB (scale each line by DB/paid),
issue (draft->open), create GRN (all lines, full qty), attach bill (mirror all PO lines +
receive links, excess on bill). Idempotent. Reuses infra from sync_po_then_attach +
call_inv from draft_issue_grn_attach.
"""
...
CAND = os.path.join(INV, "draft_inv_delivered_candidates.csv")
RESULTS = os.path.join(INV, "attach_results_draft_inv_multi.csv")
DRY_RUN = False
...
def draft_flow_multi(bill_id, po_id, db_target):
    ...
    if sorted(qkey(x) for x in bls) != sorted(qkey(x) for x in pls):
    ...
    for pl, bl in zip(pls_sorted, bls_sorted):
    ...
    if DRY_RUN:
        return ("DRY", f"sync PO {pt}->{DB} ({len(pls)} lines x{round(DB/paid,6)}); issue({po.get('status')}); "
                f"GRN all lines @ {grn_date}; attach bill {paid} (excess {round(paid-DB,2)}); inactive={len(inactive)}", bt, pt)

    has_grn = bool(po.get("purchasereceives"))
    orig_lines = [echo_line(pl, po) for pl in pls]
    orig_adj = round(float(po.get("adjustment") or 0), 2); orig_incl = bool(po.get("is_inclusive_tax"))
    for iid in inactive:
    ...
    # 1) sync PO lines to DB (scale by DB/paid) — only if no GRN and not already at DB
    if not has_grn and abs(pt - DB) > 0.05:
        scale = DB / paid
        synced = []
        for i, (pl, bl) in enumerate(zip(pls_sorted, bls_sorted), 1):
            qy = float(bl["quantity"]) or 1.0
            net_pretax_unit = float(bl.get("item_total", 0)) / qy
            synced.append({"line_item_id": pl["line_item_id"], "item_id": pl["item_id"],
                           ...
                           "description": pl.get("description", ""), "rate": round(net_pretax_unit * scale, 6),
                           "quantity": float(pl["quantity"]), "discount": 0, "unit": pl.get("unit") or bl.get("unit"),
                           "hsn_or_sac": pl.get("hsn_or_sac") or bl.get("hsn_or_sac"), "tax_id": bl.get("tax_id") or pl.get("tax_id"),
                           "location_id": pl.get("location_id") or po.get("location_id"), "item_order": i})
        ok, st, r = put_po_lines(po_id, synced, 0, False)
        if not ok:
            deact(); return "FAIL", f"PO sync PUT {st}: {r.get('message')}", bt, pt
        t1 = float(r.get("purchaseorder", {}).get("total", -1)); residual = round(DB - t1, 2)
        if abs(residual) > 5:
            put_po_lines(po_id, orig_lines, orig_adj, orig_incl); deact()
    ...
    # 3) GRN all lines (full qty) if none
    ...
    # 4) attach bill (mirror all PO lines + receive links)
...
def main():
    cands = [c for c in csv.DictReader(open(CAND, encoding="utf-8-sig")) if c["n_lines"] != "1"]
    done = set()
    ...
    for c in cands:
        ...
        n += 1
        try:
            res, reason, b2, p2 = draft_flow_multi(c["bill_id"], c["po_id"], c["db_amount"])
